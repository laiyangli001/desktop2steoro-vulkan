...
void destroy_bridge_lasers(FilamentBridge* bridge) {
...
int create_bridge_lasers(FilamentBridge* bridge) {
    if (!bridge || !bridge->engine || !bridge->scene) return 0;
    destroy_bridge_lasers(bridge);
    ...
    if (!package.isValid()) {
        set_error(bridge, "Filament could not build controller laser material");
        return 0;
    }
    bridge->laser_material = filament::Material::Builder()
            ...
            .build(*bridge->engine);
    if (!bridge->laser_material) {
        set_error(bridge, "Filament could not create controller laser material");
        return 0;
    }
    ...
    if (!bridge->laser_material_instance || !bridge->laser_vertex_buffer ||
        ...
        set_error(bridge, "Filament could not create controller laser geometry");
        return 0;
    }
    ...
    for (auto& entity : bridge->laser_entities) {
        ...
        if (result != filament::RenderableManager::Builder::Success) {
            set_error(bridge, "Filament could not create controller laser renderable");
            return 0;
        }
        bridge->scene->addEntity(entity);
        set_renderable_visible(bridge, entity, false);
    }
    return 1;
}
...
FilamentBridge* filament_bridge_create_vulkan(
    ...
    } else if (!create_bridge_lasers(bridge.get())) {
        return bridge.release();
    }
    return bridge.release();
}
...
int filament_bridge_set_controller_visible(
    ...
    for (size_t index = 0;
        ...
        const auto entity = controller.asset->getRenderableEntities()[index];
        set_renderable_visible(bridge, entity, next_visible);
    }
...
int filament_bridge_set_controller_laser(
    ...
    const filament::math::mat4f matrix(
            ...
            matrix16[12], matrix16[13], matrix16[14], matrix16[15]);
    transforms.setTransform(instance, matrix);
    set_renderable_visible(bridge, entity, true);
    return 1;
}
