...
MESSAGE_CATALOGS = {
    ...
    "Projection RCAS tooltip": "Applies RCAS after shared mip filtering and scaling. Recommended: 0.50.",
       ...
       "Preview":"Preview",
        ...
        "calibration_requires_advanced": "Automatic calibration requires Advanced Network Streaming or GPU Streaming with WebRTC.",
        ...
        "tooltip_target_fps": "Capture pacing target. Manual values stay fixed. In Auto mode, every 15 seconds the peak sustained SBS output sets capture to SBS + 5 FPS, capped by the display refresh rate. Sparse output windows are ignored.",
        ...
        "tooltip_stream_quality": "JPEG quality for Basic Network Streaming (MJPEG): 50-100. Higher values preserve more detail but increase bandwidth. Advanced and GPU streaming quality is controlled by CRF and bitrate instead.",
        "tooltip_stream_proto": "Output protocol: WebRTC is recommended for low-latency headset browsers; HLS/HLS M3U8 is browser-friendly but has higher latency; RTSP suits low-latency media players; RTMP is mainly for publishing to streaming servers.",
        ...
        "tooltip_crf": "Recommended for 4K SBS H.264 at 30 FPS, based on calibrated target bandwidth: 30 Mbps or higher -> CRF 20; 25-29 Mbps -> CRF 23; 21-24 Mbps -> CRF 26; 19-20 Mbps -> CRF 28. Below 19 Mbps, reduce resolution or frame rate instead. Lower CRF gives higher quality and requires more bitrate.",
        ...
        "tooltip_video_backend": "Streaming encoder backend: Auto keeps the stable platform path; FFmpeg uses the cross-platform hardware/software chain; Vulkan Video validates the real Vulkan H.264/HEVC profile and falls back if unavailable; PyNvVideoCodec selects NVIDIA CUDA/NVENC.",
    ...
    "Projection RCAS tooltip": "在统一 MIP 过滤和缩放后应用 RCAS 锐化。建议值：0.50。",
        ...
        "calibration_requires_advanced": "自动校准需要使用高级网络推流或 GPU 推流，并选择 WebRTC。",
        ...
        "tooltip_target_fps": "捕捉帧率节流目标。手动档位保持固定；自动模式每 15 秒按持续输出窗口内的最高 SBS 帧率评估性能，捕捉帧率设置为 SBS + 5 FPS，并限制不超过显示器刷新率；输出帧数不足的稀疏窗口不会参与校准。",
        ...
        "tooltip_stream_quality": "低级网络推流（MJPEG）的 JPEG 质量，可选 50-100；数值越高细节越好、占用带宽越大。高级网络推流和 GPU 推流改由 CRF 与码率控制，此选项不控制其编码质量。",
        "tooltip_stream_proto": "协议用途：WebRTC 适合头显浏览器，延迟最低，推荐使用；HLS/HLS M3U8 兼容普通浏览器但延迟较高；RTSP 适合低延迟播放器；RTMP 主要用于向直播服务器发布。",
        ...
        "tooltip_crf": "4K SBS、H.264、30 FPS 按自动校准目标带宽选择：30 Mbps 及以上建议 CRF 20；25-29 Mbps 建议 CRF 23；21-24 Mbps 建议 CRF 26；19-20 Mbps 建议 CRF 28。低于 19 Mbps 不建议继续提高 CRF，应降低分辨率或帧率。CRF 越低画质越高、所需码率越大。",
        "tooltip_audio_delay": "音频时间戳偏移，范围 -10 到 10 秒。正数延后音频，负数提前音频；声音领先画面时增大，声音落后画面时减小。",
        "tooltip_video_backend": "推流编码后端：自动保持当前稳定路径；FFmpeg 使用跨平台硬件/软件编码链；Vulkan Video 会真实探测 Vulkan H.264/HEVC profile，不可用时自动回退；PyNvVideoCodec 使用 NVIDIA CUDA/NVENC。",
