...
MESSAGE_CATALOGS = {
    ...
    "Projection RCAS tooltip": "Applies RCAS after shared mip filtering and scaling. Recommended: 0.50.",
       ...
       "Preview":"Preview",
        ...
        "tooltip_video_backend": "GPU-streaming encoder backend: Auto selects the suitable hardware path; FFmpeg uses its cross-platform hardware/software encoder chain; PyNvVideoCodec selects the NVIDIA CUDA/NVENC path and requires compatible NVIDIA hardware.",
    ...
    "Projection RCAS tooltip": "在统一 MIP 过滤和缩放后应用 RCAS 锐化。建议值：0.50。",
        ...
        "tooltip_video_backend": "GPU 推流编码后端：自动会选择适合当前平台的硬件路径；FFmpeg 使用跨平台硬件/软件编码链；PyNvVideoCodec 使用 NVIDIA CUDA/NVENC 路径，需要兼容的 NVIDIA 显卡。",
