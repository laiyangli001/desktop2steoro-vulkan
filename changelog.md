...
- Added an opt-in Filament controller multiview diagnostic material that replaces every controller GLB primitive with `getEyeIndex()`-selected solid red in the left eye and solid green in the right eye, isolating shader view-index routing from controller textures, lighting, and environment rendering.
