...
- CUDA/Vulkan interop 增加 FFmpeg NV12 双 plane writer：原生桥先把 FFmpeg frame 转入 `GENERAL` 并 signal timeline，CUDA 等待后把 GPU 生成的 Y/UV 直接复制到导出的 Vulkan image，再 signal 下一值给 FFmpeg 转入 `VIDEO_ENCODE_SRC`；无 CPU RGB24 或 stdin 原始帧传输。
