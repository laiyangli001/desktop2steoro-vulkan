...
## 2026-08-02
- 新增 Vulkan `surround` 环绕辉光：迁移旧工程以头部为中心的 96 段半圆柱辉光算法，复用现有 Vulkan Glow 外部纹理，并按 4×3 屏幕区域及四边颜色生成环绕光；保持 Default 环境限定和不透明输出。Filament Bridge 的 Windows、Linux、macOS 三平台构建及完整回归测试通过。
