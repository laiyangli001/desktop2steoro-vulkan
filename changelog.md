...
## 2026-08-03
- 修复 OpenXR 长时间运行后从 60Hz 降至 30Hz 并触发 GPU watchdog/runtime 异常退出，同时恢复安全的 Compute 重叠：Virtual Desktop 使用经过验证的 Filament 逐眼完成等待，不再使用会触发 GPU reset 的双眼 deferred batch 或额外 graphics queue 空提交；满足双 TensorRT 隔离槽、Triton、无 Temporal 的路径自动允许两帧在途，使 CPU 入队与上一帧 GPU 执行重叠，Presenter 一旦未及时消费输出即自动退回单帧背压。最高质量补洞不再为双眼复制 4K mask、depth 和 shift，并跳过无破洞像素的邻域采样；Vulkan 设备丢失后统一锁存终止状态，外部屏幕图像、Glow fence 和重复帧释放只清理 CPU 租约，不再向失效设备重复提交、查询或销毁 OpenXR instance。性能诊断启用时，FPSBreakdown 每 15 秒输出一次，累计 5 次后停止，覆盖 75 秒稳定性观察窗口。
