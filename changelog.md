...
## 2026-08-02
- 新增 Vulkan `surround` 环绕辉光：将背景几何扩展为连续的 96×48 前向半球网格，复用现有 Vulkan Glow 外部纹理，在 GPU 预滤波阶段计算 4×3 屏幕区域平均色并平滑插值；辉光作为整张屏幕低频颜色的 16:9 放大投影，以半球正前方区域为中心向外围连续衰减，不再从上下极点或四边分别发光。环绕壳位于房间主 View，虚拟屏幕与前景辉光位于后续 foreground View，以独立渲染 pass 保证不透明辉光不会遮挡屏幕，并保持 Default 环境限定。
