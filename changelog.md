...
## 2026-08-09
- Reduced OpenXR Projection work without changing composition order: when the rebuilt Filament Bridge exposes the new background-frame ABI, the pre-Composer pass renders only the environment and ordinary foreground, leaving controllers and guides exclusively to the existing post-Composer overlay instead of drawing them twice per eye. Older Bridge binaries retain the previous compatible path.
