...
### Vulkan / Filament multiview validation status

- **Validated VDXR array-layer routing:** one OpenXR Projection swapchain with `array_size=2`, independently cleared red in layer 0 and green in layer 1, displayed red in the left eye and green in the right eye. This proves VDXR accepts `imageArrayIndex=0/1` and rules out the closed Virtual Desktop server/client as the cause of the current same-eye output.
- **Validated the layered Filament setup but not its stereo result:** the Bridge creates one two-layer external swapchain, the Engine reports multiview stereo with two eyes, the Vulkan render pass reports `viewMask=0x3`, and controller/laser/guide programs contain the Vulkan `ViewIndex` builtin. Controller model loading remains known-good, but both eyes display the red eye-0 diagnostic, matching the earlier observation that the controller image looked two-dimensional.
- **Fixed and revalidated Filament's internal depth-clear material:** Filament 1.75 generated a multiview `clearDepth` package but registered its instanced package. The patched backend now selects `CLEARDEPTH_MULTIVIEW` for a multiview Engine; the `compiled type 1 / engine type 2` warning is gone. Both eyes remaining red after that rebuild proves the warning was a real incompatibility but not the sole stereo-routing cause.
- **Rejected swapchain readback as evidence:** scaled image blit, exact linear-image copy, and direct image-to-buffer copy all returned black for both OpenXR array layers while the headset simultaneously displayed red. Those captures do not describe the visible swapchain content and will not be used to decide whether Filament wrote layer 1 until the external-image ownership/readback boundary is separately repaired.
- **Moved the fault boundary below Filament:** a new pure Vulkan diagnostic bypasses Filament, GLB models, SBS, Glow, inference composition, and material helpers. A single full-screen multiview draw uses `viewMask=0x3` and reads `gl_ViewIndex` directly in fragment SPIR-V, yet the hardware result is still red in both eyes. Therefore further Filament changes are paused until core Vulkan multiview execution is explained.
- **Current next measurement:** the pure Vulkan fragment shader now increments a host-visible two-element counter once per view. The next run must record `fragment_counts`: `(1, 1)` sends investigation to layered attachment writes or OpenXR release/layout synchronization; `(2, 0)` proves both views execute with an incorrect zero `ViewIndex`; `(1, 0)` proves view 1 is absent and sends investigation to render-pass `pNext`/`viewMask`; `(0, 0)` indicates the diagnostic draw or SSBO visibility path failed.
- **Remaining validation order:** first make the pure Vulkan test produce and count distinct red/green views; then repeat the Filament controller eye diagnostic; only after both pass should the independent Filament offscreen producer be connected to normal screen/Glow composition. The previously device-lost dual-SwapChain deferred path remains disabled and is not part of this experiment.

## 2026-08-09
- Added a pure Vulkan multiview eye diagnostic that bypasses Filament, models, SBS, and Glow: one `viewMask=0x3` render pass draws a full-screen triangle into an OpenXR `array_size=2` swapchain and selects red/green directly from `gl_ViewIndex`. A one-pixel host-visible counter records whether fragment invocations actually execute for view indices 0 and 1 without depending on OpenXR image readback. This isolates Vulkan multiview execution from Filament after ordinary per-layer array routing passed but Filament controller output remained red in both eyes.
- Added an isolated VDXR Projection-array capability test: one `array_size=2` Vulkan Projection swapchain submits solid red layer 0 to the left view and solid green layer 1 to the right view without loading capture, inference, Filament, screen, Glow, or controllers. The dedicated PowerShell launcher validates the exact prerequisite for future Filament multiview/offscreen composition work.
- Added the next isolated Filament multiview gate after VDXR array routing passed on hardware: an opt-in launcher renders the configured environment/controllers with one stereoscopic Filament submission into one `array_size=2` Projection swapchain while disabling Vulkan SBS/Glow composition. The normal per-eye producer remains the default until this layered Filament output is visually validated.
- Fixed the Filament-only multiview diagnostic output lease: unused SBS frames are released instead of becoming the displayed frame, and the CUDA/Vulkan adapter no longer indexes release-semaphore slots for eyes that never entered source-image sampling.
- Added an opt-in Filament controller multiview diagnostic material that replaces every controller GLB primitive with `getEyeIndex()`-selected solid red in the left eye and solid green in the right eye, isolating shader view-index routing from controller textures, lighting, and environment rendering. The eye index is now read in Filament's supported vertex stage and passed to the fragment stage through a custom interpolant; the launcher also enables backend stereo tracing so View state, Vulkan render-pass view mask, and shader `ViewIndex` can be correlated with the headset result in one run.
- Added a one-shot GPU readback to the Filament multiview controller diagnostic. It saves the actual array layer 0 and layer 1 output side by side and logs each layer's non-background mean RGB, separating Filament rendering faults from OpenXR array-layer submission faults without changing the sustained rendering path. The diagnostic copies each layer into a tightly packed host-visible buffer and directly waits on Filament's finished semaphore, avoiding unreliable optimal-to-linear image copies and ambiguous intermediate synchronization on Windows Vulkan drivers.
- Fixed Filament 1.75's internal `clearDepth` material selection for multiview Engines: the patched backend now registers the generated `CLEARDEPTH_MULTIVIEW` package instead of the incompatible instanced package, removing the stereo-type mismatch from the layered controller test and keeping depth-clear geometry on the same multiview contract as the default material and skybox.
- Reduced OpenXR Projection work without changing composition order: when the rebuilt Filament Bridge exposes the new background-frame ABI, the pre-Composer pass renders only the environment and ordinary foreground, leaving controllers and guides exclusively to the existing post-Composer overlay instead of drawing them twice per eye. The controller and depth-independent guide now share one full-resolution overlay View, eliminating another per-eye post-process pass while preserving guide priority. Older Bridge binaries retain the previous compatible path.
- Fixed live hole-fill mode switching so the internal master gate changes together with the selected mode: `none` disables filling, while `balanced` and `quality` immediately restore the edge-aware path without requiring a process restart. Mode, radius, strength, and executed kernel now remain consistent when comparing fill performance at runtime.
- Fixed WindowsCaptureCUDA target-FPS enforcement with a software pacing gate before GPU buffer handling and raw-queue delivery. This compensates for capture backends that ignore `minimum_update_interval` on high-refresh displays, so selecting 60 FPS now accepts about 60 source frames per second instead of processing every 120 Hz callback; Auto continues to follow the detected display rate, and `D2S_WGC_SOFTWARE_THROTTLE=0` disables the added gate for comparison testing.
- Added render-aware OpenXR inference admission control: configured two/three-worker TensorRT pools remain available, but only one depth job may stay in flight while the Presenter is composing or already has an SBS result queued. Extra workers automatically resume when the Presenter is starved, preserving burst throughput without continuously starving Vulkan/Filament; the existing latest-frame policy and explicit worker selection remain intact.

## 2026-08-08
- Corrected the OpenXR FPS panel so SBS FPS now measures unique stereo frames actually consumed by the presenter instead of the faster inference producer rate. The OpenXR Vulkan command ring now defaults to nine slots (override with `D2S_OPENXR_VULKAN_FRAME_CONTEXTS`) so multi-pass screen, Glow, and controller/depth submissions can span all three swapchain images without prematurely reusing a three-slot fence and stalling the presenter.
- Added an opt-in, delayed SBS pacing capture for frame-timing diagnosis. After a 15-second playback setup window it records metadata for 300 new left/right Vulkan stereo outputs before Projection composition while taking only six evenly spaced SBS screenshots; sparse GPU-downscaled readback uses a bounded three-slot ring and background PNG writer, avoiding the severe throughput distortion caused by reading every output frame. The JSON manifest records source frame IDs/timestamps plus skipped or failed images, and normal inference/presentation remain unchanged when the diagnostic launcher is not used.
- Stabilized Surround Glow corners with component-wise maximum blending between the four complete shell edges: overlaps neither accumulate brightness nor let a later edge overwrite an earlier one, and no faded geometry can expose the cleared background as a black seam. Each edge grid is also reduced from 96 x 48 to 48 x 24, lowering stereo geometry from 221184 to 55296 vertices per frame. Glow now has an independent profile-driven 30 Hz update rate plus a 0.10-second GPU temporal history blend, replacing visible 2-3-frame color steps with gradual linear-color transitions without affecting controller-light sampling.
- Promoted the validated post-Composer controller pass to the normal OpenXR Vulkan default, preserving the final environment -> screen/Glow -> controller/laser/guide order without requiring a diagnostic launcher; an explicit environment override remains available for fallback testing.
...
## 2026-08-07
- Fixed Vulkan Projection Composer quality-chain composition with Filament environment and controller output: quality passes now wait for Filament completion and preserve the color target with `LOAD` before drawing the SBS screen.
