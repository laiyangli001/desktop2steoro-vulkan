...
## 2026-08-02
- 新增 Vulkan `surround` 环绕辉光：使用连续的 96×48 前向半球网格，并把半球射线反投影到当前虚拟屏幕平面，使辉光边界实时跟随屏幕宽高、距离和朝向；GPU 将屏幕划分为 8×6 区域，以最外行列作为有宽度的四边取样带，从屏幕边缘向外连续扩散。Surround 在 sRGB 感知域计算区域均值后仅解码一次进入 Filament 线性工作流，并删除固定蓝色混入，避免黑场被抬成浅蓝灰。环绕壳位于房间主 View，虚拟屏幕与前景辉光位于后续 foreground View，以独立渲染 pass 保证不透明辉光不会遮挡屏幕，并保持 Default 环境限定。
