...
## 2026-08-02
- 新增 Vulkan `surround` 环绕辉光：沿用旧工程的头部中心、屏幕边缘取色和亮度衰减，将背景几何扩展为连续的 96×48 前向半球网格；复用现有 Vulkan Glow 外部纹理，在 GPU 预滤波阶段计算 4×3 屏幕区域平均色，并在相邻区域间平滑插值，使背面颜色对应屏幕区域且不再出现硬分块。环绕壳位于房间主 View，虚拟屏幕与前景辉光位于后续 foreground View，以独立渲染 pass 保证不透明辉光不会遮挡屏幕，并保持 Default 环境限定。
