...
- 新增 Vulkan `surround` 环绕辉光：以四组互不共享极点的“屏幕边缘→半球外圈”放射条带替代经纬半球网格，每个屏幕边缘采样点沿独立球面路径均匀向四周扩散，辉光边界实时跟随屏幕宽高、距离和朝向，不再向半球上下左右中心点会聚。GPU 将屏幕划分为 8×6 区域，以最外行列作为有宽度的四边取样带，并以指数曲线形成强边缘、长距离低能量尾光；无光区域保持透明，不显示实体球体背景。Surround 在 sRGB 感知域计算区域均值后仅解码一次进入 Filament 线性工作流，并删除固定蓝色混入，避免黑场被抬成浅蓝灰；效果仍仅在 Default 环境显示。
