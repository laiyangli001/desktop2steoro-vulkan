...
## 2026-08-02
- 新增 Vulkan `surround` 环绕辉光：迁移旧工程以头部为中心的 96 段半圆柱辉光算法，复用现有 Vulkan Glow 外部纹理，并按 4×3 屏幕区域及四边颜色生成环绕光；恢复旧工程“环绕壳→虚拟屏幕→前景辉光”的明确绘制顺序，使不透明辉光不再遮挡屏幕，同时保持 Default 环境限定。Filament Bridge 的 Windows、Linux、macOS 三平台构建及完整回归测试通过。
