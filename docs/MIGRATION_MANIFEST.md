...
- `samples/`全部14个文件。
- `src/desktop2steoro/capture/`中的Python Capture实现，排除依赖Rust Native的DXGI Duplication路径。
- `src/desktop2steoro/gui/`中的Python GUI代码，排除内置Flet压缩包和本地客户端缓存。
- `src/desktop2steoro/stereo_runtime/`全部Git跟踪文件，包括Provider、模型实现、TensorRT、ROCm/MIGraphX、Triton和立体合成优化。
- `src/desktop2steoro/utils/`全部Python工具。
- `src/streaming/`中非Legacy模块。
- `src/desktop2steoro/xr_viewer/controllers/`和`src/desktop2steoro/xr_viewer/environments/`全部资产，GLB/HDR继续使用Git LFS。
- `src/desktop2steoro/xr_viewer/`中平台无关的输入、姿态、帧门控、时间和交互模块。
- Filament Bridge源码和版本清单迁移到`native/filament/`，关联GitHub Actions工作流同步迁移；多平台产物分别输出到 `src/desktop2steoro/xr_viewer/native/windows`、`linux`、`macos`。
...
- `src/desktop2steoro/capture/dxgi/native/`中的Rust扩展源码。
...
4. 独立能力探针与`src/desktop2steoro/tools/openxr_vulkan_smoke.py`实测工具。
