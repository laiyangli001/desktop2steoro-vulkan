...
- 复用 `src/desktop2stereo/viewer/vulkan_context.py` 的资源状态和 timeline 提交。
- 复用 `src/desktop2stereo/utils/screen_resolution_policy.py` 的分辨率档位策略。
...
- 保留 `src/desktop2stereo/stereo_runtime/vulkan_glow_source_pass.py` 的 Vulkan Compute 预处理。
