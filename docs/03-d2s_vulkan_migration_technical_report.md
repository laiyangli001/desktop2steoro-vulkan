...
当前已增加 CUDA/Vulkan/Filament external semaphore signal/wait ABI：支持路径不再在发布前执行 CUDA stream synchronization，Filament 在图形提交时等待对应槽位 semaphore；平台、CUDA Runtime 或旧 Bridge 不支持时自动保留 CPU 同步降级。由于 native semaphore 的消费端释放和真实 Runtime 时序仍需 Validation Layer/实机确认，默认路径使用 CPU 同步，只有显式设置 `D2S_ENABLE_CUDA_EXTERNAL_SEMAPHORE=1` 才启用异步 semaphore。Windows/Linux/macOS Bridge CI 已完成并回写最新二进制；下一阶段是 Validation Layer 和头显长稳验证。
