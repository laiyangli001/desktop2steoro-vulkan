...
| FILAMENT-002 | Filament | 单一 Filament Engine/Scene 共享 GLB、材质、纹理和 Shader；左右眼独立 Renderer、View、Camera、OpenXR swapchain 与外部屏幕纹理；虚拟屏幕可采样运行时 Vulkan 输出图像；手柄模型、按键动画、激光和逐手空闲显隐在 Projection Layer 内更新。手柄加载保留 WebXR Input Profiles GLB 原始 PBR 参数，并使用环境间接光与旧工程 profile 补光，不得由 Bridge 统一改写材质。 | 01§15.2; 02§11.1-11.5 | `src/xr_viewer/core_openxr_vulkan.py`; `src/xr_viewer/core_controller_ray.py`; `native/filament/bridge/bridge_controller.cpp`; `native/filament/bridge/bridge_material.cpp`; `filament_bridge_create_eye_swapchain`; `filament_bridge_set_active_eye`; `filament_bridge_create_screen`; `filament_bridge_set_screen_image`; `filament_bridge_load_controller`; `filament_bridge_set_controller_pose`; `filament_bridge_set_controller_inputs`; `filament_bridge_set_controller_visible`; `filament_bridge_set_controller_laser`; `filament_bridge_set_ambient_light` | `tests/test_filament_vulkan_bridge.py`; `tests/test_openxr_vulkan.py`; 单 Engine 双眼 Bridge CI；GLB 原材质/环境光/动画/虚拟屏幕/各品牌手柄、5 秒显隐、按键动画和激光实机验收 | in_progress |
