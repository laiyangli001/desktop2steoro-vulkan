...
| OPENXR-005 | OpenXR | 输出交换链色彩格式必须支持 sRGB/UNORM 可控选择，并记录运行时实际选中的格式用于 A/B 验收；默认采用已验证的 sRGB 目标，运行时中间图像保持 UNORM 存储。 | 01§8.4; 02§13.4 | `src/xr_viewer/core_openxr_vulkan.py`; `src/app_runtime/runtime_entry.py`; `src/settings.yaml` | `tests/test_openxr_vulkan.py`; 头显 sRGB 与 UNORM AB 实测 | in_progress |
