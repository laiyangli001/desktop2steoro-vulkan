...
| VK-009 | Vulkan | 左右眼使用有界多帧屏幕源图像环；每张源 `VkImage` 的 external memory、image view 和 Filament Texture 按槽位持久化缓存，并记录格式、尺寸、layout、producer/consumer queue family，正常帧不得重建、销毁或在 Filament 采样期间复用纹理。 | 01§8.2.1; 02§5.4 | `src/app_runtime/runtime_output.py`; `native/filament/bridge/bridge_internal.h`; `native/filament/bridge/bridge_screen.cpp` | `tests/test_cuda_vulkan_interop.py`; source-image barrier/ownership validation; GPU profiling; 长稳测试 | in_progress |
| VK-010 | Vulkan | CUDA/Vulkan/Filament 的屏幕源图像 producer-ready 与 consumer-release 必须通过 external semaphore/timeline 表达；source image 在 Filament 采样前转换到 `VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL`，并完成 queue ownership transfer。`set_screen_ready_semaphore` 不得代替 OpenXR 输出目标同步；CPU stream 等待只能作为显式兼容降级。 | 01§8.2.1; 01§10.1; 02§5.4; 02§6.2 | `src/viewer/cuda_vulkan_interop.py`; `src/xr_viewer/filament_vulkan_bridge.py`; native Bridge ABI; `src/viewer/vulkan_context.py` | external semaphore smoke; source-image layout/ownership Validation Layer; consumer-release reuse test; 实机长稳测试 | in_progress |
