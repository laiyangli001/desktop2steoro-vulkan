...
| VK-009 | Vulkan | 左右眼使用有界多帧输出图像环；CUDA external memory、image view 和 Filament Texture 按槽位持久化缓存，正常帧不得重建或销毁纹理。 | 01§8.2.1; 02§5.4 | `src/app_runtime/runtime_output.py`; `native/filament/bridge/filament_bridge.cpp` | `tests/test_cuda_vulkan_interop.py`; GPU profiling; 长稳测试 | verified |
...
| FILAMENT-002 | Filament | 单一 Filament Engine/Scene 共享 GLB、材质、纹理和 Shader；左右眼独立 Renderer、View、Camera、OpenXR swapchain 与外部屏幕纹理；虚拟屏幕可采样运行时 Vulkan 输出图像；手柄模型、按键动画、激光和逐手空闲显隐在 Projection Layer 内更新。 | 01§15.2; 02§11.1-11.5 | `src/xr_viewer/core_openxr_vulkan.py`; `src/xr_viewer/core_controller_ray.py`; `filament_bridge_create_eye_swapchain`; `filament_bridge_set_active_eye`; `filament_bridge_create_screen`; `filament_bridge_set_screen_image`; `filament_bridge_load_controller`; `filament_bridge_set_controller_pose`; `filament_bridge_set_controller_inputs`; `filament_bridge_set_controller_visible`; `filament_bridge_set_controller_laser` | `tests/test_filament_vulkan_bridge.py`; `tests/test_openxr_vulkan.py`; 单 Engine 双眼 Bridge CI；GLB/动画/虚拟屏幕/各品牌手柄、5 秒显隐、按键动画和激光实机验收 | in_progress |
...
| FILAMENT-005 | Filament | 每眼维护运行时 VkImage 到 Filament Texture 的持久化缓存；Projection Layer 两眼先提交、帧边界统一等待并统一 release。 | 01§8.2.1; 02§5.5 | `native/filament/bridge/filament_bridge.cpp`; `src/xr_viewer/core_openxr_vulkan.py` | Bridge CI; OpenXR 帧时序测试; 实机 FPS/Validation 测试 | verified |
