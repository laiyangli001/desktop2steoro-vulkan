...
| VK-009 | Vulkan | 左右眼使用有界多帧屏幕源图像环；每张源 `VkImage` 的 external memory、image view 和 Filament Texture 按槽位持久化缓存，并记录格式、尺寸、layout、producer/consumer queue family，正常帧不得重建、销毁或在 Filament 采样期间复用纹理。 | 01§8.2.1; 02§5.4 | `src/app_runtime/runtime_output.py`; `native/filament/bridge/bridge_internal.h`; `native/filament/bridge/bridge_screen.cpp` | `tests/test_cuda_vulkan_interop.py`; `tests/test_openxr_vulkan.py`; `src/tools/vulkan_transfer_smoke.py`; GPU profiling; 长稳测试 | in_progress |
| VK-010 | Vulkan | GPU producer（CUDA、ROCm/HIP 等）/Vulkan/Filament 的屏幕源图像 producer-ready 与 consumer-release 必须通过 external semaphore/timeline 表达；source image 在 Filament 采样前转换到 `VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL`，并完成 queue ownership transfer；Filament per-eye render-finished 必须通过稳定 ABI 交给 release submit。`set_screen_ready_semaphore` 不得代替 OpenXR 输出目标同步；CPU/host stream 等待只能作为显式兼容降级。 | 01§8.2.1; 01§10.1; 02§5.4; 02§6.2 | GPU producer adapter; `src/xr_viewer/filament_vulkan_bridge.py`; `filament_bridge_get_finished_drawing_semaphore`; native Bridge ABI; `src/viewer/vulkan_context.py` | `tests/test_cuda_vulkan_interop.py`; `tests/test_openxr_vulkan.py`; `tests/test_filament_vulkan_bridge.py`; 外部 semaphore smoke; 实机长稳测试 | in_progress |
...
| GRAPH-004 | 计算图 | NVIDIA CUDA 与 AMD ROCm/Windows 使用同一套 Triton 立体合成 kernel；CUDA 12.8 profile 使用 PyTorch 2.11 + Triton 3.6，ROCm7 profile 保留独立 nightly 配套版本，真实 kernel 探测失败或厂商不支持时选择 Vulkan Compute。 | 01§6; 02§10 | `src/stereo_runtime/triton_runtime.py`; `src/stereo_runtime/fast_plus_fused_triton.py`; `src/stereo_runtime/hole_fill_triton.py`; `src/stereo_runtime/occlusion_triton.py`; `src/stereo_runtime/compute_backend.py`; `src/requirements-cuda.txt`; `src/requirements-rocm7.txt` | `tests/test_compute_backend.py`; `src/tools/probe_triton_backend.py`; 实机探测; Vulkan smoke | in_progress |
...
| FILAMENT-004 | Filament | 主场景在 Rec709/D65 线性工作空间渲染，ColorGrading 使用 `LINEAR` tone mapping，不得使用 ACES/其他场景曲线；场景曝光进入 ColorGrading，不修改 glTF 材质颜色。屏幕、UI 和激光进入同一主 View，屏幕按 sRGB 图像导入并只解码一次；保持后处理只用于最终 sRGB 输出目标的单次编码。 | 01§6.3.1; 01§8.2.2; 02§8.3; 02§11.2.1 | `native/filament/bridge/bridge_material.cpp`; `native/filament/bridge/bridge_context.cpp`; `native/filament/bridge/bridge_eye.cpp`; `native/filament/bridge/bridge_screen.cpp`; `native/filament/bridge/bridge_laser.cpp` | `tests/test_filament_vulkan_bridge.py`; `tests/test_openxr_vulkan.py`; `.github/workflows/filament-bridge.yml` | implemented |
| FILAMENT-005 | Filament | 每眼维护运行时屏幕源 `VkImage` 到 Filament Texture 的持久化缓存；源图像 ready、layout/ownership transfer、Filament sampling completion 和生产端 release 必须独立于 OpenXR 输出 swapchain 同步；Projection Layer 两眼先提交、帧边界统一等待并统一 release。能力不满足时自动回退一次 GPU copy/Quad Layer。所有图形资源操作必须由 Presenter 线程通过命令队列执行，后台线程不得直接调用 Filament C ABI 或操作 Vulkan 资源。 | 01§8.2.1; 02§5.4-5.5; 02§13.1 | `native/filament/bridge/bridge_internal.h`; `native/filament/bridge/bridge_eye.cpp`; `native/filament/bridge/bridge_screen.cpp`; `src/xr_viewer/core_openxr_vulkan.py` | `tests/test_filament_vulkan_bridge.py`; `tests/test_openxr_vulkan.py`; OpenXR 帧时序测试; 实机 FPS/Validation 测试 | in_progress |
