...
| VK-009 | Vulkan | 左右眼使用有界多帧输出图像环；CUDA external memory、image view 和 Filament Texture 按槽位持久化缓存，正常帧不得重建或销毁纹理。 | 01§8.2.1; 02§5.4 | `src/app_runtime/runtime_output.py`; `native/filament/bridge/filament_bridge.cpp` | `tests/test_cuda_vulkan_interop.py`; GPU profiling; 长稳测试 | implemented |
| VK-010 | Vulkan | CUDA/Vulkan/Filament 的 producer-ready 与 consumer-release 必须通过 external semaphore/timeline 表达；CPU stream 等待只能作为显式兼容降级。 | 01§10.1; 02§6.2 | `src/viewer/cuda_vulkan_interop.py`; `src/xr_viewer/filament_vulkan_bridge.py`; native Bridge ABI | external semaphore smoke; Validation Layer; 实机长稳测试 | in_progress |
...
| FILAMENT-002 | Filament | 单一 Filament Engine/Scene 共享 GLB、材质、纹理和 Shader；左右眼独立 Renderer、View、Camera、OpenXR swapchain 与外部屏幕纹理；虚拟屏幕可采样运行时 Vulkan 输出图像。 | 01§15.2; 02§11.1-11.5 | `src/xr_viewer/`; `filament_bridge_create_eye_swapchain`; `filament_bridge_set_active_eye`; `filament_bridge_create_screen`; `filament_bridge_set_screen_image`; `filament_bridge_load_controller`; `filament_bridge_set_controller_pose`; `filament_bridge_set_controller_inputs` | 单 Engine 双眼 Bridge CI；GLB/动画/虚拟屏幕/各品牌手柄与按键动画实机验收 | in_progress |
...
| FILAMENT-004 | Filament | Preview 与 OpenXR 必须使用同一套 profile 光照参数、ACES 色调映射和 Rec709/sRGB/D65 输出色彩空间；Filament 输出保持线性 Rec709，Projection Layer 的 sRGB 目标负责唯一一次 sRGB transfer；运行时屏幕纹理按 sRGB 采样。 | 01§7; 01§8.4; 02§11; 02§13.4 | `native/filament/bridge/filament_bridge.cpp`; `src/xr_viewer/filament_preview_bridge.py`; `src/xr_viewer/filament_vulkan_bridge.py`; `src/xr_viewer/core_openxr_vulkan.py`; `src/app_runtime/output_contract.py` | 三平台 Bridge CI；Preview 与 OpenXR 亮度色彩 AB 实测 | in_progress |
| FILAMENT-005 | Filament | 每眼维护运行时 VkImage 到 Filament Texture 的持久化缓存；Projection Layer 两眼先提交、帧边界统一等待并统一 release。 | 01§8.2.1; 02§5.5 | `native/filament/bridge/filament_bridge.cpp`; `src/xr_viewer/core_openxr_vulkan.py` | Bridge CI; OpenXR 帧时序测试; 实机 FPS/Validation 测试 | in_progress |
...
| OPENXR-005 | OpenXR | 输出交换链色彩格式必须支持 sRGB/UNORM 可控选择，并记录运行时实际选中的格式用于 A/B 验收；Projection Layer 默认使用已验证的 sRGB 目标，Filament 输出保持线性 Rec709，运行时中间图像保持 UNORM 存储。 | 01§8.4; 02§13.4 | `src/xr_viewer/core_openxr_vulkan.py`; `src/app_runtime/runtime_entry.py`; `src/settings.yaml` | `tests/test_openxr_vulkan.py`; 头显 sRGB 与 UNORM AB 实测 | in_progress |
