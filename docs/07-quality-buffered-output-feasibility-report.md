...
在当前 Desktop2Stereo 架构中，新增一个可选的“高画质缓冲输出”模式是可行的，但实时模式不能承诺每个输入帧都完成完整深度和补洞。低端 GPU 上必须采用有界缓冲、关键帧完整处理和中间帧 temporal/warp 传播。

该模式不应覆盖现有实时模式，而应作为 GUI 中的独立勾选项启用。启用后，系统允许输出端落后捕获端一段可配置时间，用帧缓存和前后帧 lookahead 给深度估计、视差生成、遮挡补洞、时间稳定提供更充足的计算窗口。它适合看视频、电影、本地播放器、非交互式桌面内容；不适合游戏、鼠标精确操作、低延迟 VR 交互。若用户要求每个输入帧都完整处理，应使用离线渲染模式，而不是实时 OpenXR 模式。
...
输入帧 -> 有界 ring buffer
       -> 关键帧完整深度/补洞
       -> 中间帧 temporal/warp 传播
       -> OpenXR 72Hz 输出
```

缓冲必须有硬上限，不能通过无限积压来伪造实时处理能力：

- 缓冲延迟限制在 500ms～2s，由用户选择或场景预设决定。
- 缓冲满时，优先丢弃尚未开始处理的中间帧，保留关键帧和当前时间连续性。
- 实时路径禁止使用硬盘缓存；只允许 GPU resident buffer 和有界系统内存 ring buffer。
- 无法维持延迟上限时，明确切换到降帧或降质量策略，不能继续增长队列。
- 输出调度依据 presentation timestamp，而不是处理完成即立即显示。
...
其中 `present_fps` 可以保持 OpenXR 的显示提交频率，但 `processed_fps` 反映真实生成了多少张新内容帧，避免把重复显示误认为完整推理帧率。

## 为什么缓冲能提升画质

单纯“延迟显示”只能吸收算力抖动，不能自动提升画质。实时质量缓冲的核心是“有界延迟 + 关键帧完整处理 + 中间帧传播”；在吞吐不足时，不能要求每个输入帧都走完整补洞路径。lookahead 只能作为后续画质增强，不能替代吞吐控制。
...
在有界缓冲和允许额外延迟时，可以在输出第 N 帧时使用未来帧：
...
实时质量缓冲模式的超限策略应进一步细化为：

```text
1. 丢弃尚未开始处理的中间帧。
2. 保留关键帧以及最新可展示帧。
3. 若仍无法恢复延迟上限，切换到降帧或降质量。
4. 禁止无限扩大 ring buffer，禁止写入硬盘作为实时后备缓存。
```
...
- 使用有界 ring buffer 按 timestamp 缓存输入和 runtime result。
- 实现固定延迟 presentation scheduler。
- 增加关键帧完整处理和中间帧 temporal/warp 传播接口。
- 缓冲满时优先丢弃尚未开始处理的中间帧。
- 增加降帧/降质量 fallback，禁止无限堆积。
- OpenXR、本地输出共用同一缓冲层。
- 不改变默认实时模式。
- 暴露 `buffer_depth`、`input_fps`、`processed_fps`、`present_fps`。
...
- 缓冲帧数有上限。
- 显存不足时可降级而不是崩溃。
- `present_fps` 可维持目标显示频率时，必须同时显示真实的 `processed_fps`，不得将帧复用计入完整处理帧率。
...
第一版不要急于实现最复杂的 future-frame 补洞。应先完成可选开关、有界 ring buffer、关键帧完整处理、中间帧 temporal/warp 传播、本地/OpenXR 双输出、延迟和吞吐统计。确认 presentation 层稳定并验证超限降级后，再逐步引入 lookahead 补洞和独立音频链路。
