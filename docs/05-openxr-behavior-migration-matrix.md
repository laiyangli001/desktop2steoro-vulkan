...
| 🧪 | `effects._render_glow`、`_render_frost_glow`、`_render_frost_veil` | `_update_filament_glow`、`VulkanGlowSourceComputeBackend`、`d2s_glow_source.comp`、`bridge_glow.cpp` | 复用 v2.5 的五态、矩形 SDF、LOD/MIP 足迹、Frosted/Veil 公式和 premultiplied alpha；CUDA 源经 external buffer+semaphore 交给同 family 的独立 Vulkan Compute queue，生成 320x180 线性 RGBA8 外部图像；只绑定已完成槽位，未完成时复用上一帧，不阻塞主屏 zero-copy；能力不足时才回退 CPU 小纹理 | Filament shader compile、`test_vulkan_glow_source.py`、GPU source/release-order tests | 五种效果、曲面屏、右 Grip+左摇杆透明度、12 Hz 更新、主屏帧率和两小时长稳 |
