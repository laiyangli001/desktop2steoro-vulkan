...
| ⚠️ | `implementation` 中 X/Y/A/B 分支 | `_update_x_shortcuts`、`_dispatch_controller_shortcut` | X 长按的 Default 光圈/环境光分支、Y 预设、A 曲面、B 背景；当前 X 仍只覆盖 Filament 灯光/天空盒，旧 glow 状态未接入 | 快捷键状态测试 | Default、房间环境、旁路透视分别验证 |
...
| ⚠️ | `base`、`environment_effects`、`effect_scheduler` glow/frosted | Filament bridge scene/effect API | 当前只有 `_filament_glow_*` 状态，没有旧工程完整 glow/frosted 计算与渲染路径 | 待建立 effect state test | Default 光圈/环境灯光实测 |
