...
虚拟屏幕、UI 和手柄激光属于显示参考 LDR 内容：其输入图像必须是 `VK_FORMAT_R8G8B8A8_SRGB`，Filament 只解码一次；它们必须进入关闭后处理的独立 View，不得经过主场景 ACES/其他 tone mapping。手柄模型必须以原始 PBR 材质保留在 HDR 主 View；激光 View 使用由同一 controller GLB 创建的独立遮挡实例，该实例与主实例共享纹理、材质资源和顶点缓冲，但拥有独立 Renderable，并从加载起永久绑定 `colorWrite=false`、`depthWrite=true` 的深度遮挡材质。禁止在两个 View 的异步提交之间临时修改同一 Renderable 的材质绑定，也不得为遮挡把手柄彩色 PBR 本体移入无后处理 View。默认颜色控制必须为恒等变换，非中性颜色控制只能来自明确的用户配置。
...
Filament Bridge 必须将渲染对象按颜色语义和遮挡关系分层：HDR 场景层使用带 ColorGrading 的主 View，并以 GLB 原始 PBR 材质渲染手柄；显示参考屏幕、UI 和激光使用无后处理 View。为实现激光遮挡，AssetLoader 必须从同一 controller GLB 建立两个资源共享实例：主实例仅在 HDR View 输出原始 PBR 颜色，遮挡实例仅在 LDR View 以持久深度材质写深度。两个实例同步根姿态、按键动画和显隐，但不得在帧提交期间互换材质。主 View 不得渲染 LDR 图元，LDR View 不得输出手柄颜色或应用 ACES/其他场景 tone mapping。`VK_FORMAT_R8G8B8A8_SRGB` 是当前屏幕纹理导入的唯一合法格式；以 UNORM 图像配合 `SRGB8_A8` 采样属于错误，必须拒绝。
