...
虚拟屏幕、UI 和手柄激光属于显示参考 LDR 内容：其输入图像必须是 `VK_FORMAT_R8G8B8A8_SRGB`，Filament 只解码一次；它们必须进入关闭后处理的独立 View，不得经过主场景 ACES/其他 tone mapping。手柄模型与激光必须进入同一个深度测试 View，使手柄外壳遮挡位于其后的光束；屏幕和说明 UI 使用后续显示 View，避免说明端点被手柄反向遮挡。默认颜色控制必须为恒等变换，非中性颜色控制只能来自明确的用户配置。
...
Filament Bridge 必须将渲染对象按颜色语义和遮挡关系分层：HDR 场景层使用带 ColorGrading 的主 View；手柄与激光共享一个无后处理、启用深度写入和深度测试的 View；显示参考屏幕与 UI 使用后续无后处理 View。主 View 不得渲染 LDR 图元，LDR View 不得应用 ACES 或其他场景 tone mapping。`VK_FORMAT_R8G8B8A8_SRGB` 是当前屏幕纹理导入的唯一合法格式；以 UNORM 图像配合 `SRGB8_A8` 采样属于错误，必须拒绝。
...
手柄激光属于与控制器同坐标系的 3D 场景几何体，随左右眼 View 分别投影；手柄与激光必须共享同一深度附件和深度测试，使外壳遮挡光束根部。它不是平面 UI，不得占用 OpenXR Quad Layer。
