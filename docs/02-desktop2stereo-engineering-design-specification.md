...
虚拟屏幕、UI 和手柄激光属于显示参考 LDR 内容：其输入图像必须是 `VK_FORMAT_R8G8B8A8_SRGB`，Filament 只解码一次；它们必须进入关闭后处理的独立 View，不得经过主场景 ACES/其他 tone mapping。手柄模型必须以原始 PBR 材质保留在 HDR 主 View；激光 View 复用同一手柄几何，但仅临时绑定 `colorWrite=false`、`depthWrite=true` 的深度遮挡材质，先写入外壳深度再测试激光，并在该 View 提交后立即恢复原材质。不得为遮挡把手柄彩色 PBR 本体移入无后处理 View。默认颜色控制必须为恒等变换，非中性颜色控制只能来自明确的用户配置。
...
Filament Bridge 必须将渲染对象按颜色语义和遮挡关系分层：HDR 场景层使用带 ColorGrading 的主 View，并以 GLB 原始 PBR 材质渲染手柄；显示参考屏幕、UI 和激光使用无后处理 View。为实现激光遮挡，手柄几何同时加入 LDR View，但只在该 View 渲染期间临时使用不写颜色、只写深度的遮挡材质，随后恢复原材质。主 View 不得渲染 LDR 图元，LDR View 不得输出手柄颜色或应用 ACES/其他场景 tone mapping。`VK_FORMAT_R8G8B8A8_SRGB` 是当前屏幕纹理导入的唯一合法格式；以 UNORM 图像配合 `SRGB8_A8` 采样属于错误，必须拒绝。
...
手柄激光属于与控制器同坐标系的 3D 场景几何体，随左右眼 View 分别投影；激光 View 必须使用控制器同一份外壳几何作为仅写深度的遮挡体，使外壳遮挡光束根部，同时控制器颜色仍仅由 HDR 主 View 的原始 PBR 材质输出。它不是平面 UI，不得占用 OpenXR Quad Layer。
