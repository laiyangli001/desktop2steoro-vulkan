...
OpenGL 路径的能力探测必须验证实际 context、纹理格式、PBO/fence、CUDA–OpenGL interop、厂商编码器和目标分辨率，不能仅凭 `OpenGL` 字符串或显卡名称选择。NVIDIA 且 CUDA graphics interop 成功时，CUDA RGBA tensor 通过带 `cudaGraphicsRegisterFlagsSurfaceLoadStore` 能力的 `cudaGraphicsGLRegisterImage` 映射 OpenGL RGBA8 texture。高级推流现在优先加载原生 `d2s_nvenc_cudaarray_bridge`，以 `NV_ENC_INPUT_RESOURCE_TYPE_CUDAARRAY` 和 `NV_ENC_BUFFER_FORMAT_ABGR` 直接注册该持久映射的 `CUarray`，压缩包继续复用 FFmpeg mux-only 与 MediaMTX 发布层；原生 DLL 缺失、驱动 API 不兼容或注册失败时，才释放持久映射并回退 PyNvVideoCodec。

严格零拷贝阶段由原生 `cudaarray_surface_writer.cu` 完成：Python 只传递最终 SBS tensor 的 device pointer、真实 shape/byte-stride、`uint8/float32/float16` 类型和当前 CUDA stream；kernel 直接读取该 tensor，并通过 `surf2Dwrite` 写入 NVENC 已注册的 CUDAARRAY，不再创建 RGBA staging，也不调用 `cudaMemcpy*`。kernel 在生产 tensor 的 stream 上提交，编码前显式 `cudaStreamSynchronize`，保证独立 NVENC 队列不会读取未完成的表面。这条链路实际记录 `gpu_to_cpu=False zero_copy=True gpu_copy_count=0`；这里的“零拷贝”指没有 CPU 边界、没有跨 API 图像复制，RGBA 格式整理是直接写编码表面的融合 kernel。

NVIDIA 回退顺序固定为三级：直接 surface kernel 失败后，使用 `cudaMemcpy2DToArrayAsync` 写同一 CUDAARRAY，记录 `zero_copy=False gpu_copy_count=1`；原生 CUDAARRAY 注册或编码失败后，再进入 PyNvVideoCodec 的 array→linear 兼容路径，记录 `zero_copy=False gpu_copy_count=2`。[PyNvVideoCodec 官方 GPU 编码接口](https://docs.nvidia.com/video-technologies/pynvvideocodec/pynvc-api-prog-guide/using_pynvvideocodec_apis.html)要求 NV12 plane 提供 CUDA Array Interface 设备指针，不能直接接收这里的 `cudaArray_t`。ROCm/HIP 环境继续使用同一 graphics-resource ABI 映射 OpenGL texture，并交给已有 AMF surface 编码器；该路径仍记录 `gpu_to_cpu=False zero_copy=False gpu_copy_count=2`。没有 CUDA/HIP interop、使用 Intel/macOS、输入本身已经是 CPU RGB，或互操作探针失败时，先完成 OpenGL context/纹理/PBO/fence 能力探测，再直接回退现有 host-upload，明确记录 `interop=none gpu_to_cpu=True` 和 `host-upload fallback`；CPU/非 CUDA 帧通过 `VulkanDirectSbsOutput.submit_frame()` 进入同一回退边界，不会错误调用 Vulkan rawvideo 编码命令；由于此时原始帧已经在 CPU，不能再把它上传 OpenGL 后读回，否则只会增加无意义的 CPU↔GPU 往返。当前项目没有把 OpenGL texture 伪装成 QSV/VAAPI surface，也没有把它伪装成 VideoToolbox 的 IOSurface-backed `CVPixelBuffer`；这些需要各平台原生硬件帧和进程内编码桥，不能由 FFmpeg rawvideo stdin 实现。OpenGL 路径发生运行时错误后熔断本次会话并回退 host-upload，避免 Vulkan/OpenGL 之间来回抖动。
...
- 性能测速不得被请求输出 FPS 自身封顶：测速阶段捕获临时使用“请求输出 FPS + 5 FPS”，统计推理后可交付的 SBS 生产率；若实测达到或超过请求上限，编码目标使用完整请求值（例如实测 ≥30 时输出 30 FPS），否则再按安全余量向下选择稳定档位。测速结束后，手动捕获恢复配置值，自动捕获保持“所选编码 FPS + 5 FPS”的生产余量。
...
- 自动校准得到的 30 FPS 只写入高级网络推流专属的 `Stream Target FPS`；本地 Viewer、3D Monitor 和 OpenXR 继续使用独立的 `Target FPS`。网络校准不得覆盖、同步或间接限制非网络模式的捕获与 SBS 帧率。
- 每个新码率档位必须单独计时 15 秒；计时起点是头显浏览器首次上报正的 `decoded_fps`、确认画面已经开始解码显示的时刻。码率切换、WHEP 重连、等待首帧和 `decoded_fps=0` 期间不得计入测试窗口；显示中断后必须重新从下一次成功解码开始计时。
...
- 只有用户点击运行时，高级推流才重新读取当前选择显示器的实际分辨率，并将其归一化记录为 `1K`、`2K` 或 `4K` 档位；档位与已保存 fingerprint 不一致时，必须先完成自动校准，不能直接开始正常推流。用户在 GUI 中临时切换选项但未点击运行时，不得触发校准失效提示。
...
校准浏览器页面（默认由本机校准服务端口提供，当前端口为 `1123`）必须在测试期间明确显示当前测试档位：目标码率、峰值码率、测试帧率、阶段名称和阶段进度；同时显示浏览器侧实时接收码率、解码 FPS、丢帧和丢包。校准完成后，页面必须将提示切换为最终结果，至少包含稳定网络上限、安全目标码率、安全峰值码率、最终 FPS 和实测接收码率，不能只显示“测试完成”或最后一档测试值。
